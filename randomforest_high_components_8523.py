...
# Load Dataset
df = pd.read_csv('crypto_data.csv.csv')  # Replace with your dataset
df['Date'] = pd.to_datetime(df['Date'])
df.set_index('Date', inplace=True)

# Feature Engineering (Without TA-Lib)
...
features = df[['Open', 'High', 'Low', 'Volume', 'SMA_7', 'SMA_21', 'EMA_7', 'EMA_21', 'RSI', 'MACD', 'MACD_signal', 'BB_Upper', 'BB_Lower']]
target = df['Close']
...
# Make Predictions
y_pred_scaled = rf_final.predict(X_test)
